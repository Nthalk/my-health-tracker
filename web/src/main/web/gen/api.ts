...
export interface CheckIn {
    coldSweatsScale: number;
    congestionScale: number;
    feverStartAt: Date;
    gpsAccuracy: number;
    gpsLatitude: number;
    gpsLongitude: number;
    sensitiveSkinScale: number;
    soreThroatScale: number;
    temperature: number;
    token: string;
}
...
export class HealthCheckApiClient {
    ...
    checkIn(arg0: CheckIn): RestResponse<ApiResult<Unit>> {
        return this.httpClient.request({ method: "POST", url: uriEncoding`api/health-check/check-in`, data: arg0 });
    }
}
